...
export async function exportClientsExcel() {
  const [cliRes, livRes] = await Promise.all([
    supabase
      .from('profiles')
      .select('id, nom, prenom, nom_societe, email, telephone, ville, livreur_id, actif, created_at, notes_internes')
      .eq('role', 'client')
  ...
  const headers = ['Société', 'Nom', 'Prénom', 'Email', 'Téléphone', 'Ville', 'Livreur', 'Actif', 'Créé le', 'Notes internes'];
  const widths = [28, 18, 18, 30, 16, 18, 20, 8, 12, 40];
